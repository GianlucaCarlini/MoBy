...
def dict_to_list(config: dict) -> list:
    """converts a dictionaries in a config to lists

    Parameters
    ----------
    config : dict
        The config for which the dictionaries have to be converted

    Returns
    -------
    config : dict
        The converted config with lists instead of dictionaries
    """
    for key, value in config.items():
        if isinstance(value, dict):
            config[key] = [item for item in value.values()]
        else:
            config[key] = value
...
def init_from_config(
    ...
    if dicts_to_lists:
        new_config = dict_to_list(new_config)
